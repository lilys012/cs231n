...
def svm_loss(x, y):
    ...
    margin = x - correct_scores.reshape(-1, 1) + 1
    mask = (margin > 0) * 1
    loss = np.sum(margin * mask) / num_train - 1
    cntnum = -1 * (np.sum(mask, axis=1) - 1)
    mask[np.arange(num_train), y] = cntnum
    dx = mask / num_train
...
def softmax_loss(x, y):
    ...
    #shift = np.exp(x - np.max(x, axis=1).reshape(-1, 1))
    #prob = shift / np.sum(shift, axis = 1).reshape(-1, 1)
